...
class TestCase:
    ...
    def find(request):
        ...
        title = title.strip()
        remark=parameter.get("remark") if parameter.get("remark", "") else ""
        remark=remark.strip()
        ...
        end_time = parameter.get("endTime", now) if parameter.get("endTime", now) else now
        try:
            tcs = testcase.objects.filter(createTime__lt=end_time,createTime__gt=start_time,remark__contains=remark,
            ...
            # if remark:
            #     tcs = tcs.filter(remark__contains=remark)
            total = len(tcs)
        except:
            return JsonResponse(400, "时间参数错误")
