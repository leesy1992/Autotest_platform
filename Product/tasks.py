...
def SplitTaskRunning(splitResult_id):
    from Product.models import SplitResult, Browser, Environment, Element, Check, Result, EnvironmentLogin, LoginConfig,Keyword
    ...
    for step in steps:
        try:
            ...
            split.remark="测试用例执行第" + str(index) + "步:"+keyname+ elemetname
            log.info(split.remark)
...
def timingRunning():
    ...
    from Autotest_platform.helper.util import get_model
    from Autotest_platform.PageObject.Base.PageObject import sleep
